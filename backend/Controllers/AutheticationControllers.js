...
const TechnicianSignUp = async (req, res) => {
    try {
        ...
        const profileImageFilename = req.files.profileImage[0].filename;
        const certificateFilename = req.files.certificateDocument[0].filename;

        // Create file URLs
        const imageUrl = `${req.protocol}://${req.get('host')}/uploads/images/${profileImageFilename}`;
        const certificate = `${req.protocol}://${req.get('host')}/uploads/certificatesdocs/${certificateFilename}`;
        ...
        const newTechnician = await prisma.$transaction(async (tx) => {
            ...
            // Create technician details - use imageUtl which matches the prisma schema field
    ...
    return res.status(201).json({
        ...
        user: userWithoutPassword,
        technicianDetails: newTechnician.techDetails,
        token
    });
